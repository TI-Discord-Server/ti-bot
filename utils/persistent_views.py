...
class PersistentViewManager:
    ...
    async def store_view_message(
        ...
        try:
            ...
            logger.info(
                f"Stored persistent view: {view_type} in channel {channel_id}, message {message_id}"
            )
        except Exception as e:
            logger.error(f"Failed to store persistent view {view_type}: {e}")

    async def get_view_messages(
        self, view_type: str = None, guild_id: int = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def remove_view_message(self, channel_id: int, message_id: int):
        ...
        try:
            ...
            if result.deleted_count > 0:
                logger.info(
                    f"Removed {result.deleted_count} persistent view(s) for message {message_id}"
                )
        except Exception as e:
    ...
    async def restore_views(self):
        ...
        try:
            ...
            try:
                ...
                if count == 0:
                    logger.info(
                        "No persistent views found to restore (first run or empty collection)"
                    )
                    return
            except Exception as e:
                logger.info(
                    f"Persistent views collection doesn't exist yet or is inaccessible: {e}"
                )
                return
            ...
            logger.info(
                f"Persistent view restoration complete: {restored_count} restored, {failed_count} failed"
            )
    ...
    async def _restore_single_view(self, view_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Message {message_id} not found for view {view_type}, removing from database"
            )
    ...
    async def _create_view(
        self, view_type: str, guild_id: int, additional_data: Dict[str, Any]
