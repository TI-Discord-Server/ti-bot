...
class DatabaseMigrator:
        ...
        try:
            ...
            for record in email_records:
                try:
                    ...
                    migrated_count += 1
                    print(f"{'[DRY RUN] Would migrate' if dry_run else 'Migrated'} email hash for user {record['_id']}")
                    
                except Exception as e:
                    print(f"Error migrating email record for user {record.get('_id', 'unknown')}: {e}")
                    continue
            ...
            for record in warn_records:
                try:
                    ...
                    new_record = {
                        ...
                        "moderator_id": record.get("staffmember", 0),  # Staff member who issued the warn
                        "type": "warn",
                    ...
                    migrated_count += 1
                    print(f"{'[DRY RUN] Would migrate' if dry_run else 'Migrated'} warn {record['_id']} for user {record['userID']}")
    ...
    async def verify_migration(self):
        ...
        try:
            ...
            new_warn_count = await self.new_db["infractions"].count_documents({"type": "warn", "old_warn_id": {"$exists": True}})
