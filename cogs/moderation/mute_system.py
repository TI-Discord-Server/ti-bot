...
class MuteSystem:
    ...
    async def execute_mute(
        ...
        dm_embed = create_dm_embed(
            "⚠️| Je bent gemute.", f"Reden: {reason}", discord.Color.dark_gray(), bot_icon_url
        )
    ...
    async def execute_scheduled_mute(
        self,
        interaction: discord.Interaction,
        member: discord.Member,
        reason: str,
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"{member.mention} is gemute"
                + (f" voor {duration}" if scheduled else "")
                + f". Reden: {reason}",
                color=discord.Color.green(),
            )
            ...
            try:
                ...
                self.bot.log.error(
                    f"Failed to log mute infraction for {member.name} ({member.id}): {e}"
                )

        except discord.errors.Forbidden as e:
            self.bot.log.error(
                f"Permission denied when trying to mute {member.name} ({member.id}): {e}"
            )
            embed = discord.Embed(
                title="Permissie Fout",
            ...
            self.bot.log.error(
                f"Unexpected error when trying to mute {member.name} ({member.id}): {e}",
                exc_info=True,
            )
            embed = discord.Embed(
            ...
            if has_muted_role:
                ...
                try:
                    ...
                    if scheduled_unmute:
                        ...
                        current_punishment_info.append(
                            f"**Muted Role** {time_info} (scheduled unmute: {discord.utils.format_dt(unmute_time, 'F')} - {discord.utils.format_dt(unmute_time, 'R')})"
                        )
                    else:
                        current_punishment_info.append(
                            f"**Muted Role** {time_info} (permanent/manual unmute required)"
                        )
    ...
    async def execute_mute_with_cleanup(
        self,
        interaction: discord.Interaction,
        member: discord.Member,
        reason: str,
        ...
        if member.timed_out_until and member.timed_out_until > discord.utils.utcnow():
            try:
                ...
                self.bot.log.error(
                    f"Failed to remove timeout from {member.name} ({member.id}): {e}"
                )
