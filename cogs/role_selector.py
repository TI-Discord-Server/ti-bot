...
class RoleCategory:
    ...
    def from_dict(cls, data: Dict):
        return cls(name=data.get("name", ""), roles=data.get("roles", []))
        ...
        try:
            ...
            try:
                ...
                await interaction.followup.send(
                    "Er is een fout opgetreden. Probeer het opnieuw.", ephemeral=True
                )
            except Exception as followup_error:
                self.role_selector.bot.log.error(
                    f"Failed to send error message via followup: {followup_error}"
                )
        ...
        try:
            ...
            if not category:
                await interaction.response.send_message(
                    "Deze categorie bestaat niet meer.", ephemeral=True
                )
                return
            ...
            if roles_to_add:
                try:
                    ...
                    await interaction.response.send_message(
                        "Ik heb geen toestemming om rollen toe te voegen.", ephemeral=True
                    )
                    return
                except Exception as e:
                    self.role_selector.bot.log.error(f"Error adding roles: {e}")
                    await interaction.response.send_message(
                        "Er is een fout opgetreden bij het toevoegen van rollen.", ephemeral=True
                    )
                    return
            ...
            if roles_to_remove:
                try:
                    ...
                    await interaction.response.send_message(
                        "Ik heb geen toestemming om rollen te verwijderen.", ephemeral=True
                    )
                    return
                except Exception as e:
                    self.role_selector.bot.log.error(f"Error removing roles: {e}")
                    await interaction.response.send_message(
                        "Er is een fout opgetreden bij het verwijderen van rollen.", ephemeral=True
                    )
                    return
...
class RoleSelector(commands.Cog):
    def __init__(self, bot):
        ...
        self.role_selector_cache = (
            {}
        )  # key: cache_key, value: {"embed": embed, "view": view, "timestamp": timestamp}
        ...
        self.default_categories = [
            ...
            RoleCategory(
                "Updates", [{"name": "Low Priority", "emoji": "✅", "role_name": "Low Priority"}]
            ),
        ]
    ...
    async def get_categories(self) -> List[RoleCategory]:
        ...
        return [
            RoleCategory.from_dict(category) for category in categories_doc.get("categories", [])
        ]
    ...
    async def _get_cache_key(self, category_name: str, user_roles: List[discord.Role]) -> str:
        ...
        try:
            ...
            user_category_roles = sorted(
                [role.name for role in user_roles if role.name in category_role_names]
            )
                ...
                if self.bot.persistent_view_manager:
                    try:
                        ...
                        self.bot.log.warning(
                            f"Failed to store persistent view for role selector: {e}"
                        )
    ...
    async def show_role_select(
        self, interaction: discord.Interaction, category_name: str, message: str = None
    ):
            ...
            try:
                ...
                asyncio.create_task(
                    self._refresh_cache_in_background(
                        cache_key, category_name, interaction.user.roles
                    )
                )
                return
            except Exception as e:
                self.bot.log.warning(
                    f"Failed to use cached role selector, falling back to full build: {e}"
                )
        ...
        async def build_role_selector():
            try:
                ...
                back_button = discord.ui.Button(
                    label="Terug naar categorieën",
                    style=discord.ButtonStyle.secondary,
                    custom_id="back_button_show_role_select",
                )
                ...
                view.add_item(
                    RoleSelect(self, category_name, category.roles, interaction.user.roles)
                )
                ...
                for role in category.roles:
                    user_has_role = any(
                        user_role.name == role["role_name"] for user_role in interaction.user.roles
                    )
                ...
                if role_list:
                    embed.add_field(
                        name="Beschikbare Rollen", value="\n".join(role_list), inline=False
                    )

                embed.set_footer(
                    text="Gebruik het dropdown menu om rollen te selecteren/deselecteren"
                )
                ...
                try:
                    await interaction.followup.send(
                        "Er is een fout opgetreden bij het laden van de rolselectie.",
                        ephemeral=True,
                    )
    ...
    async def update_role_select_message(
        self,
        ...
        if not category:
            if interaction.response.is_done():
                ...
                await interaction.response.send_message(
                    "Deze categorie bestaat niet.", ephemeral=True
                )
            return
        ...
        back_button = discord.ui.Button(
            label="Terug naar categorieën",
            style=discord.ButtonStyle.secondary,
            custom_id="back_button_update_role_select",
        )
        ...
        async def back_button_callback(back_interaction: discord.Interaction):
            try:
                ...
                self.bot.log.error(
                    f"Error in back button callback (update_role_select_message): {e}"
                )
                try:
                    await back_interaction.followup.send(
                        "Er is een fout opgetreden. Probeer het opnieuw.", ephemeral=True
        ...
        if role_list:
            embed.add_field(name="Beschikbare Rollen", value="\n".join(role_list), inline=False)
        ...
        try:
            ...
            if e.code == 10062:  # Unknown interaction
                self.bot.log.warning(
                    "Interaction expired (10062) in role selector, cannot update message"
                )
            else:
    ...
    async def _create_role_select_view(
        self, category_name: str, user_roles: List[discord.Role]
    ) -> discord.ui.View:
        ...
        back_button = discord.ui.Button(
            label="Terug naar categorieën",
            style=discord.ButtonStyle.secondary,
            custom_id="back_button_create_role_select_view",
        )
    ...
    async def _refresh_cache_in_background(
        self, cache_key: str, category_name: str, user_roles: List[discord.Role]
    ):
