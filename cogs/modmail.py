...
class Modmail(commands.Cog, name="modmail"):
    ...
    async def close(
        ...
        if thread is None:
            await interaction.response.send_message(
                "❌ Dit is geen actief ticket kanaal.", ephemeral=True
            )
            return
        ...
        try:
            ...
            if modmail_logs_channel_id is None:
                ...
                await interaction.followup.send(
                    "⚠️ Waarschuwing: Geen modmail log kanaal geconfigureerd. Ticket wordt gesloten maar transcript wordt niet gelogd.",
                    ephemeral=True,
                )
            else:
                try:
                    ...
                    await interaction.followup.send(
                        "⚠️ Waarschuwing: Het geconfigureerde modmail log kanaal bestaat niet meer. Ticket wordt gesloten maar transcript wordt niet gelogd.",
                        ephemeral=True,
                    )
                except discord.Forbidden:
                    await interaction.followup.send(
                        "⚠️ Waarschuwing: Geen toegang tot het modmail log kanaal. Ticket wordt gesloten maar transcript wordt niet gelogd.",
                        ephemeral=True,
                    )
            ...
            await thread.close(
                ...
                message=reason,
                silent=silent,
                log_channel=modmail_logs_channel,
            )
            self.bot.log.info(
                f"Modmail ticket closed successfully by {interaction.user.name} ({interaction.user.id}) in channel {interaction.channel.id}. Reason: {reason or 'No reason provided'}"
            )
        except discord.Forbidden as e:
            error_msg = f"❌ Geen toestemming om ticket te sluiten: {str(e)}"
            self.bot.log.error(
                f"Permission denied when closing modmail ticket in channel {interaction.channel.id}: {e}"
            )
            ...
            self.bot.log.error(
                f"Channel or user not found when closing modmail ticket in channel {interaction.channel.id}: {e}"
            )
            await interaction.followup.send(error_msg, ephemeral=True)
            return
            ...
            self.bot.log.error(
                f"Discord API error when closing modmail ticket in channel {interaction.channel.id}: {e}"
            )
            await interaction.followup.send(error_msg, ephemeral=True)
            return
            ...
            self.bot.log.error(
                f"Unexpected error when closing modmail ticket in channel {interaction.channel.id}: {e}",
                exc_info=True,
            )
            await interaction.followup.send(error_msg, ephemeral=True)
            return
    ...
    async def generate_transcript(
        ...
        try:
            await thread.store_and_send_log(
                closer=interaction.user, log_channel=interaction.channel
            )
            confirmation = await interaction.followup.send(
                "📝 Transcript gegenereerd", ephemeral=True
            )
            await asyncio.sleep(2)
            await confirmation.delete()
        except Exception as e:
            await interaction.followup.send(
                f"❌ Fout bij genereren transcript: {str(e)}", ephemeral=True
            )
    ...
    async def transcripts(
        ...
        if not files_list:
            await interaction.followup.send(
                f"Geen transcripts gevonden voor {user.mention} ({user.display_name})."
            )
            return
        ...
        for file in files_list:
            ...
            discord_file = discord.File(
                file_data,
                filename=f"{counter}_transcript_{file_id}_{timestamp.strftime('%Y-%m-%d_%H:%M')}.html",
            )
        ...
        for batch in file_batches:
            await interaction.followup.send(
                f"{amount} transcripts gevonden voor {user.mention} ({user.display_name}):",
                files=batch,
            )
    ...
    async def reply(self, interaction: discord.Interaction, msg: str):
        ...
        try:
            ...
            await interaction.followup.send(
                f"❌ Fout bij verzenden bericht: {str(e)}", ephemeral=True
            )
            return
    ...
    async def edit(
        ...
        message_id = (
            int(message_id) if message_id.isdigit() and 17 <= len(message_id) <= 19 else None
        )
    ...
    async def contact(self, interaction: discord.Interaction, user: discord.Member | discord.User):
        ...
        category = discord.utils.get(
            self.bot.guild.categories, id=await self.get_modmail_category_id()
        )
        errors = []
        ...
        if errors or not user:
            ...
            if manual_trigger:  # not react to contact
                embed = discord.Embed(
                    title=title, color=discord.Color.red(), description="\n".join(errors)
                )
