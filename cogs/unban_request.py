...
class UnbanView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Unban Aanvragen", style=discord.ButtonStyle.green, custom_id="unban_aanvraag_knop"
    )
...
class UnbanAanvraagModal(discord.ui.Modal, title="Unban Aanvraag"):
    ...
    banreden = discord.ui.TextInput(
        label="Waarom was je geband?",
        style=discord.TextStyle.long,
        required=True,
        placeholder="Geef de reden van je ban.",
    )
    ban_datum = discord.ui.TextInput(
        label="Wanneer was je geband?",
        style=discord.TextStyle.short,
        required=True,
        placeholder="Geef de datum van je ban in DD/MM/YYYY formaat.",
    )
    reden = discord.ui.TextInput(
        label="Waarom denk je dat je een unban verdient?",
        style=discord.TextStyle.long,
        required=True,
        placeholder="Geef een duidelijke uitleg.",
    )
    berouw = discord.ui.TextInput(
        label="Wat heb je geleerd van de ban?",
        style=discord.TextStyle.long,
        required=True,
        placeholder="Beschrijf hoe je gedrag zal veranderen.",
    )
    toevoeg = discord.ui.TextInput(
        label="Wil je nog iets toevoegen?",
        style=discord.TextStyle.long,
        required=False,
        placeholder="Schrijf hier een opmerking of vraag.",
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        infractions = (
            await self.bot.db.infractions.find(
        ...
        embed.add_field(
            name="Waarom denk je dat je een unban verdient?", value=reden_antwoord, inline=False
        )
        ...
        try:
            kanaal1 = (
                self.bot.get_channel(int(self.aanvragen_log_kanaal_id_1))
                if self.aanvragen_log_kanaal_id_1
                else None
            )
        ...
        try:
            kanaal2 = (
                self.bot.get_channel(int(self.aanvragen_log_kanaal_id_2))
                if self.aanvragen_log_kanaal_id_2
                else None
            )
        except (ValueError, TypeError):
            kanaal2 = None
        ...
        if kanaal2:
            await kanaal2.send(embed=embed)
        elif self.aanvragen_log_kanaal_id_2:
            self.bot.log.warning(
                f"Archive kanaal met ID {self.aanvragen_log_kanaal_id_2} niet gevonden."
            )

        await interaction.followup.send(
            "Je unban aanvraag is succesvol verzonden.  We zullen je aanvraag zo snel mogelijk beoordelen.",
            ephemeral=True,
        )
...
class UnbanRequest(commands.Cog):
    ...
    async def load_unban_settings(self):
        ...
        if settings:
            self.unban_request_kanaal_id = (
                str(settings.get("unban_request_kanaal_id"))
                if settings.get("unban_request_kanaal_id")
                else None
            )
            self.aanvragen_log_kanaal_id_1 = (
                str(settings.get("aanvragen_log_kanaal_id_1"))
                if settings.get("aanvragen_log_kanaal_id_1")
                else None
            )
            self.aanvragen_log_kanaal_id_2 = (
                str(settings.get("aanvragen_log_kanaal_id_2"))
                if settings.get("aanvragen_log_kanaal_id_2")
                else None
            )
