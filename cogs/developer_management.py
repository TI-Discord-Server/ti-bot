        ...
        try:
            ...
            try:
                ...
                await interaction.response.send_message(
                    "❌ Ongeldig ID formaat. Voer een geldig Discord ID in.", ephemeral=True
                )
                return
            ...
            if dev_id in dev_ids:
                await interaction.response.send_message(
                    f"❌ Gebruiker met ID `{dev_id}` is al een ontwikkelaar.", ephemeral=True
                )
                return
            ...
            if not user:
                try:
                    ...
                    await interaction.response.send_message(
                        f"❌ Geen gebruiker gevonden met ID `{dev_id}`.", ephemeral=True
                    )
                    return
            ...
            await self.bot.db.settings.update_one(
                {"_id": "server_settings"}, {"$set": {"developer_ids": dev_ids}}, upsert=True
            )
            ...
            await interaction.response.send_message(
                "❌ Er is een fout opgetreden bij het toevoegen van de ontwikkelaar.",
                ephemeral=True,
            )
...
class RemoveDeveloperSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not removed_devs:
                await interaction.response.send_message(
                    "❌ Geen geldige ontwikkelaars geselecteerd.", ephemeral=True
                )
                return
            ...
            embed.add_field(
                name="👨‍💻 Verwijderde Ontwikkelaars", value="\n".join(removed_devs), inline=False
            )
            ...
            await interaction.response.send_message(
                "❌ Er is een fout opgetreden bij het verwijderen van ontwikkelaars.",
                ephemeral=True,
            )
...
class RemoveDeveloperView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message(
                "Je hebt geen toestemming om deze actie uit te voeren.", ephemeral=True
            )
            return False
        return True
...
class DeveloperManagementView(discord.ui.View):
    ...
    async def remove_developer(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if not dev_ids:
                await interaction.response.send_message(
                    "❌ Geen ontwikkelaars om te verwijderen.", ephemeral=True
                )
                return
    ...
    async def refresh(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.response.send_message(
                "❌ Er is een fout opgetreden bij het vernieuwen.", ephemeral=True
            )
